...
$max_file_size = 409600; // 400 kb in bytes//1048576; // 1 MB expressed in bytes
$countUp=0;
...
$upload_path = "F:/InstallXampp/htdocs/FileUpload/file_uploads";
// Define allowed filetypes to check against during validations
$allowed_mime_types = ['text/plain', 'image/jpg', 'image/jpeg'];
...
function upload_file($field_name) {
	global $upload_path, $max_file_size, $allowed_mime_types, $allowed_extensions, $check_is_image,$check_for_php,$countUp;
	...
	if(isset($_FILES[$field_name])) {
		...
		if($error > 0) {
		...
		} elseif($check_is_image && (getimagesize($tmp_file) === false)&&!in_array($file_extension, $allowed_extensions) ) {
			...
			// You obviously would only run this check if expecting an image.		              
                    ...
                    echo "Error: Not a valid file.<br />";
		}               
